...
class AuthenticationFailedError(Exception):
    ...
    def __init__(self, msg: str = None, reason: str = None):
        self.msg = msg or "Authentication failed. " + reason or ""
...
class InvalidHttpMethodError(Exception):
    ...
    def __init__(self, msg: str = None):
        self.msg = (
            msg
            or "Method is not valid. Method must be POST, PUT, GET or DELETE in a string format"
        )
...
class HttpCodeError(Exception):
    ...
    def __init__(self, response=None, msg: str = None):
        if response is None and msg is None:
            raise Exception(
                "Neither response nor message for creating HttpCodeError was passed."
            )
